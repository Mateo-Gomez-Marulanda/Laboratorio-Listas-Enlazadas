...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        listaJugadores.consultarTurno("Juan");
...
class ListaDobleCircular<T> {
    ...
    public boolean consultarTurno (T valor) {
        NodoDoble<T> aux = nodoPrimero;
        int cont = 0;
        while (cont < tamanio) {
            if (aux.getValorNodo().equals(valor)) {
                NodoDoble<T> aux2 = aux.getAnteriorNodo();
                NodoDoble<T> aux3 = aux.getSiguienteNodo();
                System.out.println("El turno de " + aux.getValorNodo() + " Va después del turno del jugador " + aux2.getValorNodo() + " y antes del turno del jugador " + aux3.getValorNodo());
                return true;
            } else {
                aux = aux.getSiguienteNodo();
                cont++;
